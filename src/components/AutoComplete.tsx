...
export default function AutoComplete({ options, value, onChange }: any) {
  ...
  const filteredOptions = options.filter((option: any) => option.includes(value));
  ...
  return (<div className="relative w-64 " ref={ref}>
    ...
    <ul className={`absolute w-full rounded-lg shadow-lg ${!showOptions && 'hidden'} select-none`}>
      {filteredOptions.length > 0 ? filteredOptions.map((option: any, i: number, arr: []) => {
