...
export const calculateItemPrice = (product: CartProduct, price: number): number => {
...
export const formatter = (currency, digits = 0) => new Intl.NumberFormat(currency.format, {
  style: "currency",
...
export const cartAnimation = event => {
  ...
  let disRight = imgtodrag.getBoundingClientRect().right;
  let disTop = imgtodrag.getBoundingClientRect().top;
  let cartleft = viewcart.getBoundingClientRect().left;
  let cartRight = viewcart.getBoundingClientRect().right;
